/*
[S10AP1] Twitter
    Após a popularidade do Orkut, outra rede social se tornou popular por aqui. Aparentemente limitada, a rede social do
    passarinho permitia apenas publicações em texto. Cada publicação podia ter, no máximo, 144 caracteres. Para cada
    publicação, além de seu conteúdo, deve ser armazenado também a identificação do usuário. O usuário poderia excluir
    qualquer publicação de sua autoria e adicionar novas publicações sem se preocupar com limitações. Implemente, na
    linguagem C, um programa que simula o funcionamento dessa rede social, considerando que o programa deve exibir um
    menu em que o usuário pode optar por:

        I) inserir uma nova publicação, digitando o seu conteúdo;
        II) visualizar as suas publicações;
        III) excluir uma publicação feita por ele.
*/
